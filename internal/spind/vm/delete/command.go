...
func New(options *Options, runtime *cliruntime.Runtime) *cobra.Command {
	...
	command.Flags().BoolVar(&options.Force, "force", false, "Stop running VMs before deleting them.")
	command.Flags().BoolVar(&options.UnmergeKubeconfig, "unmerge-kubeconfig", false, "Remove spind kubeconfig entries for deleted VMs.")
	return command
}
